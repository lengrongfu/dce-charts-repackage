...
imageRegistry=`sed -n -e '/registry: m.daocloud.io\\/docker.io\\/library/=' harbor-operator/values.yaml`
os=$(uname)
echo $os
...
if [ $os == "Darwin" ];then
  ...
  registry: m.daocloud.io\\/docker.io\\/library
    ...
    if [ ! $imageRegistry ];then
            line=`sed -n -e '/# image.repository/=' harbor-operator/values.yaml`
            echo "line is:" $line
            sed -i "$line i \  registry: m.daocloud.io\\/docker.io\\/library" harbor-operator/values.yaml
    fi
fi
